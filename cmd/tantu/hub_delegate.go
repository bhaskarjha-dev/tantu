...
func delegateSendWithName(webAddr, filePath, text, name, targetPeer string, timeout time.Duration) error {
	return delegateSendWithNameFromStore("", webAddr, filePath, text, name, targetPeer, timeout)
}

func delegateSendWithNameFromStore(storeDir, webAddr, filePath, text, name, targetPeer string, timeout time.Duration) error {
	if webAddr == "" {
	...
	if err != nil {
		return fmt.Errorf("marshal text delegation body: %w", err)
	}
...
func delegateFileWithName(client *http.Client, storeDir, webAddr, filePath, name, targetPeer string) error {
	...
	if err != nil {
		...
		<-writeErr
		return fmt.Errorf("create file delegation request: %w", err)
	}
	...
	if pipeErr := <-writeErr; pipeErr != nil {
		return fmt.Errorf("prepare delegation upload: %w", pipeErr)
	}
	return nil
}

func doDelegationRequest(client *http.Client, req *http.Request) error {
	resp, err := client.Do(req)
	if err != nil {
