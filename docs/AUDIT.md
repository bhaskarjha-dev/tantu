...
| DO-02 | File finalization removed an existing destination before rename and could report false success. | All receiver paths publish through private staging and no-overwrite hard-link/exclusive-copy finalization before acknowledging success. |
| FS-01 | `.part` resumption was keyed only by filename and could combine unrelated files. | Drop IDs are constrained to safe identifiers and metadata/checksum checks were tightened; a durable cross-process transfer manifest is still recommended. |
...
- Protocol frames are capped at 4 MiB, and control frames now have a 64 KiB typed limit; aggregate per-peer byte/session quotas are still needed. The current caps and session limits reduce, but do not eliminate, aggregate memory pressure from many malformed or concurrent data frames.
- `Envelope` schemas still need stricter duplicate/unknown-field validation and explicit version negotiation.
- Peer-store writes are unique-temp/atomic and metadata is private, but Windows ACLs, cross-process load-modify-rename locks, and crash recovery need work.
- Peer resolution is deterministic and re-pairing preserves aliases/defaults, but the active peer selection is still process-local rather than persisted.
...
- SSE has no replay/`Last-Event-ID` mechanism and drops events for slow clients. It is bounded to 32 subscribers with non-blocking broadcasts.
- The embedded dashboards still use inline scripts for compatibility; migrate dynamic actions to DOM event listeners and remove `unsafe-inline` when the UI is refactored.
- Standalone relay/drop servers now enforce loopback/origin checks, bounded bodies/history/sessions, and a per-process mutation token. Hub control handlers also have bounded bodies/read deadlines; browser-level tests and a CSP migration remain.
...
- Durable resume manifests, aggregate transfer quotas, and explicit browser-level security tests remain open hardening work.
...
- invalid/hostile DropID, kind, name, size, and chunk metadata; short writes; finalization failure; no-overwrite publication; and unsafe filename handling;
...
The current Windows toolchain cannot run the race runtime (`-race` requires a complete cgo/race installation); CI should run race tests on every supported platform. The release job should pin Go and GoReleaser versions rather than using floating `latest` values.

## Release recommendation

Do not describe the current build as “MITM-proof,” “zero external exposure,” or “strictly 5 GiB” until the remaining high-priority items are closed. The repeat-OAuth regression, callback redaction/correlation, authenticated local control plane, runtime ownership/lease checks, generation-safe lifecycle, dynamic-port pairing, rejected-pairing reporting, pinned LAN dials, explicit SSH authentication, fatal-listener handling, and pre-ack file publication are fixed and tested. Remaining release blockers for untrusted-network use are durable cross-process resume/identity manifests, aggregate quotas/typed data-frame budgets, complete SSH deployment UX, OS ACL/PID hardening, mixed-version probe compatibility, and removal or formal acceptance of inline-script CSP debt.
